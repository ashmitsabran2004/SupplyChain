...
| Frontend bundle | Partial | The app lazy-loads `MapView`, which imports Mapbox GL ESM on demand; loading states cover API warm-up and map loading. Vite splits React and Mapbox core/shared chunks. Production build succeeded, but still reports Mapbox chunks over the default 500 kB limit: `mapbox-core` 775.20 kB and `mapbox-shared` 587.02 kB (minified). The initial app and map component chunks are small; the remaining warning is in deferred Mapbox code. |
...
| Frontend completeness | Done | `frontend/src/App.tsx` shows `/impact` dollar exposure and SLA count, risk-density heat and node layers, consumes WebSocket ticks, refreshes selected-node impact/cascade for live ticks, and displays backend-ready status. |
...
- UI and bundle config: `frontend/src/App.tsx`, `frontend/src/MapView.tsx`, `frontend/src/api.ts`, `frontend/vite.config.ts`
